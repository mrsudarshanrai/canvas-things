...
for (let i = 0; i < numberOfDots; i++) {
    lotsOfDot.push(new Dots(Math.floor(Math.random() * window.innerWidth), Math.floor(Math.random() * window.innerHeight)));
}
...
const detectCollision = (dot1, dot2, index, i) => {
    if (dot1.x + 100 >= dot2.x &&
        dot1.x <= dot2.x + 100 &&
        dot1.y + 100 >= dot2.y &&
        dot1.y <= dot2.y + 100) {
        makeLines(index, i);
